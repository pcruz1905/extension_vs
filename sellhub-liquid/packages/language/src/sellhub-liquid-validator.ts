...
export class SellhubLiquidValidator {
   ...
   */
  async checkIslandComponentExists(
    islandTag: IslandTag,
    ...
    try {
      ...
      if (!manifest.components.includes(componentName)) {
        accept(
          "error",
          `Unknown component "${componentName}". Component not found in manifest.`,
          {
            node: islandTag,
            property: "name",
          }
      ...
      console.error(
        "SellhubLiquidValidator: Failed to validate component existence:",
        error
      );
    }
  }
   ...
   */
  async checkIslandRequiredProps(
    islandTag: IslandTag,
    accept: ValidationAcceptor
  ): Promise<void> {
    const componentName = this.extractComponentName(islandTag);
    ...
    try {
      ...
      console.error(
        "SellhubLiquidValidator: Failed to validate required props:",
        error
      );
    }
  }
  ...
  checkIslandHydrateValue(
    ...
    if (!validStrategies.includes(hydrateValue)) {
      accept(
        "error",
        `Invalid hydration strategy "${hydrateValue}". Must be one of: ${validStrategies.join(
          ", "
        )}`,
        {
          node: islandTag,
